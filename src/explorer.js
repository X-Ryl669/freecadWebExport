var { xeogl } = require('./glTFModel.js') 

function GLTFExplorer (menuId, modelId, ghosted, fixColors) {
    ...
    model.on("loaded", function () {
        ...
        for (var objectId in model.objects) {
            if (model.objects.hasOwnProperty(objectId)) {
                ...
                if (!("material" in object)) object.material = new xeogl.Material;
...
if (!window.GLTFExplorer) {
  window.GLTFExplorer = GLTFExplorer;
}
